---
page_title: "apn_interface"
subcategory: ""
description: |-
  This represents an APN (Access Point Name) interface, which is used for Access Point for modem interface. It allows the configuration of various Modem settings such as Access Point Name, PPPoE user name, PPPoE password, PPPoE service name.
---

# apn_interface (Nested-Attribute)
...
- `password` (String) The PPPoE password used for authentication when the interface is configured for PPPoE.
...
- `username` (String) The PPPoE username used for authentication when the interface is configured for PPPoE.
